		...
		int maxLen, int hapaxLength)
{
    ...
    printf("Hapax legomena from file '%s':\n", filename);
     ...
     // using the same logic as the printData function but I added another if condition on line 81 and another condition on line
	 // 84 that checks if the word on occurs once
    for (int i = 0; i < maxLen; i++) {
        currentNode = wordListHeads[i];
        if (currentNode != NULL) {
            
            if (!(hapaxLength >= 0 && i != hapaxLength)) {
            
                while (currentNode != NULL) {
                    if (currentNode->value == 1) { // checking that a word only appears once, then print otherwise go to next node
                        printf("    %s\n", currentNode->key);
                    }
...
main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		if (argv[i][0] == '-') 
		{
			...
			//printf("About to call tallyWordsInFile for %s\n", argv[i]);
